...
Under your extracted folder, `Mod_Name\WAD` you will find all of your mod's files compressed within a single WAD file, labeled `Champion.wad.client` or `Champion.en_US.wad.client`. Extract a WAD by dragging it onto one of CSLOL's tools called `wad-extract`, found in your CSLOL directory under `CSLOL\cslol-tools\wad-extract`. This extracts your mod's files into a folder called `Champion.wad` next to your WAD file. It is usually necessary to have updated hashes in your `wad-extract` directory in order for your files to have their full pathnames. Outdated hashes will not prevent you from converting or editing your files, but without them you may inadvertantly convert files to TEXT you otherwise had not meant to, such as any QWER+P icons. This being because they are largely indistingushable while not in their directory.
